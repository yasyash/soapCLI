...
- `int soap_PATCH_ns__BriefData(struct soap*, const char *URL, const ns__BriefData*)` REST PATCH XML, returns `SOAP_OK` or error code
...
The following operations on `ns__SensorData` are available:
...
The component of XML schema type *`ns:ArrayOfSensorData`* in schema "[urn:ReplicationApiService](#doc-namespaces)" is:
...
This class is declared in [replicator.h](replicator.h) at line 112, is serialized as XML schema type *`ns:GetSensorsResponse`*, and has the following auto-completed declaration in soapStub.h:
...
The component of XML schema type *`ns:GetHistoricalDataBriefResponse`* in schema "[urn:ReplicationApiService](#doc-namespaces)" is:
...
- `int soap_read_ns__ArrayOfSensors(struct soap*, ns__ArrayOfSensors*)` deserialize from XML, returns `SOAP_OK` or error code
...
<tr><td><code> std::vector<ns__BriefData *>  </code></td><td><code> std__vectorTemplateOfPointerTons__BriefData </code></td><td> container </td><td> ns:BriefData </td></tr>
</table>
...
This service operation is declared in [replicator.h](replicator.h) at line 141 and has the following properties:
...
This service operation is declared in [replicator.h](replicator.h) at line 142 and has the following properties:
...
### Service Operation `ReplicationApiServiceService::GetHistoricalDataBrief()`

This service operation is declared in [replicator.h](replicator.h) at line 143 and has the following properties:
...
- `std::wstring login` is required
- `std::wstring password` is required
- `int AveragePeriod` is required
- `ns__ArrayOfSensors sSensors` is required, where the type of this parameter is <code><a href="#ns__ArrayOfSensors"> ns__ArrayOfSensors </a></code>
- `std::wstring From` is required
- `std::wstring To` is required
- `ns__GetHistoricalDataBriefResponse &result` is the service operation response data populated by the service operation, where the type of this result parameter is <code><a href="#ns__GetHistoricalDataBriefResponse"> ns__GetHistoricalDataBriefResponse </a></code>
...
_Generated on Thu Jan 31 2019 22:57:07 UTC by soapcpp2 v2.8.75 for replicator.h._
